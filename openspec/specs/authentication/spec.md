...
## Purpose
Defines how Workout Agent authenticates API requests and manages sessions across server and mobile. It covers Better Auth (anonymous sessions with optional email/password upgrade), deterministic auth-mode selection with a CE/dev stub fallback and fail-closed hosted mode, capability discovery via `/api/meta`, and the `userId` vs `principalId` identity model with strict token/credential secrecy.
