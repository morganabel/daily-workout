...
#### Scenario: Google capability is explicit

- **GIVEN** the server has both Google OAuth credentials configured
- **WHEN** a client fetches `/api/meta`
- **THEN** the response sets `auth.googleAvailable=true`
- **AND** the response sets `auth.googleAvailable=false` when either credential is missing
...
Users MUST be able to register using email and password or Google OAuth when that provider is configured. If the user is currently signed in anonymously, the system SHALL upgrade/link that anonymous principal to the provided credentials, preserving the same `userId` for metering/billing continuity.
...
#### Scenario: Anonymous-to-email upgrade rolls back atomically

- **GIVEN** an anonymous user is being upgraded through a newly-created email user
- **WHEN** any credential transfer, session transfer, temporary-user deletion, or final profile update fails
- **THEN** none of those database changes are committed
- **AND** the credential and session remain attached to the newly-created email user
...
#### Scenario: Google OAuth upgrade preserves userId

- **GIVEN** an anonymous user with `userId=abc123`
- **AND** Google OAuth is configured
- **WHEN** they complete Google sign-in
- **THEN** the resulting account retains `userId=abc123`
- **AND** the Google account is linked to that user
- **AND** the user's generated anonymous email, name, and image are replaced by the verified Google profile

#### Scenario: Incomplete Google OAuth is not reported as success

- **GIVEN** a user starts Google OAuth
- **WHEN** they cancel the browser or the callback does not establish a session and linked Google account
