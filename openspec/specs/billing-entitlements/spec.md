...
Hosted generation policy MUST evaluate entitlements before model invocation for requests that use server-managed or Vertex credentials. Requests that exceed included limits MUST be denied with `QUOTA_EXCEEDED` and MUST NOT invoke model providers. Entitlement bypass applies only when the credential actually selected for the chosen provider came from a matching client BYOK header; BYOK remains subject to platform admission controls.
...
#### Scenario: Unused key cannot bypass entitlement quota

- **GIVEN** the account has exhausted included usage and supplies a provider-specific key that is not selected for the chosen provider
- **WHEN** generation would use a managed credential
- **THEN** policy returns `QUOTA_EXCEEDED` before provider invocation
