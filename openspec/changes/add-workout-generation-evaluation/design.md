...
Evaluation runs will declare provider choice, run count, and review mode up front. CE workflows should work with env keys, BYOK, or mock fallback for plumbing checks. Hosted-oriented evaluation must surface missing-key, quota, and estimated usage issues before launching broad runs.
