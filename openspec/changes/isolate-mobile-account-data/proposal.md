...
No production users or durable mobile data need to be preserved. This change can replace the unsafe contracts directly without carrying migration, cleanup, quarantine, compatibility, or staged-rollout machinery for the old unscoped local database.
...
- Start directly with principal-scoped storage without inspecting or handling an old unscoped database.

## Capabilities

### New Capabilities

- `mobile-data-isolation`: Defines principal-owned SQLite partitions, repository lifecycle, and scoped secure settings without a legacy database path.
...
- Data impact: new app data is principal-scoped. The implementation does not inspect, migrate, or delete a legacy unscoped database because there are no released users to upgrade.
