...
The app has no production users. We can establish the correct ownership and workout-targeting invariants directly and avoid legacy migration, cleanup, and rollout scaffolding for the old unscoped database.
...
- Detecting or deleting a legacy unscoped database.
...
### 8. Scoped SQLite Does Not Handle A Legacy Database

The app has no released users of the unscoped database, so scoped SQLite starts directly with per-principal database names. It does not detect, open, migrate, quarantine, or delete an old unscoped database, and it needs no database cleanup marker. Development fixtures and tests seed data into an explicitly resolved scope.
...
| M3  | Principal-scoped SQLite factory, repository injection, and observer/debug isolation                                                  | M2                               |
...
3. Merge M3, which switches the app to scoped database construction without a legacy database path.
...
There is no legacy database cleanup, data backfill, compatibility window, staged rollout, dual-write period, or rollback to the unscoped schema.
