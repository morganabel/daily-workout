...
- [x] 1.1 Add shared contract fields needed for planning-date context, explicit regeneration baseline workout input, and minimal regeneration provenance
- [x] 1.2 Update request/plan validation tests to cover the new planning and regeneration contract fields without breaking existing canonical `TodayPlan` usage
- [x] 1.3 Ensure mobile and server request builders can carry full merged context during regeneration

## 2. Planning Brief, Smart Resolution, And Server Lifecycle

- [x] 2.1 Implement server-side `PlanningBrief` derivation from request, merged context, planning date, and optional baseline workout
- [x] 2.2 Implement deterministic Smart-focus resolution for recommended intent, disallowed stressors, and coarse load ceilings
- [x] 2.3 Normalize generation inputs, block intents, variation rules, and fallback policy without changing the public `TodayPlan` response contract
- [x] 2.4 Add unit tests for planning-brief derivation, unknown handling, and Smart-resolution behavior across recent-session and upcoming-event cases
...
- [x] 4.1 Update provider prompting inputs to consume the planning brief and candidate-pool context instead of raw request/context payloads
- [x] 4.2 Implement provider-aware regeneration routing so stateful continuity is used only when valid and stateless regeneration uses explicit baseline workout data otherwise
- [x] 4.3 Update mobile regeneration submissions and server handling so full context and baseline workout data are available for both OpenAI and Gemini flows
