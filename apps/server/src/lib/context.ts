...
import { isAutoFocus } from './ai-providers/utils';
...
export async function loadGenerationContext(
  ...
  if (request.context) {
    ...
    if (result.success) {
      ...
      return {
        ...
        environment: {
          ...
          timeAvailableMinutes: request.timeMinutes ?? result.data.environment.timeAvailableMinutes,
        },
        preferences: {
    ...
    console.warn('[context] Invalid client context, building from request:', result.error.message);
  }
